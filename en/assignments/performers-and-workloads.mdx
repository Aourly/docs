...
description: "Learn how to add team members to assignments, set individual rates, and configure daily workload schedules for accurate time tracking."
---

Performers are the people who do the work on an assignment. In Aourly, every person who logs time against an assignment must be added as a performer. This guide covers how to add performers, set their rates, and configure workload schedules.

## What Is a Performer?

A performer is a team member assigned to work on a specific assignment. Each performer has:
- Their own **hourly rate** for the assignment
...
For most sole proprietors using Aourly, you will be the only performer on your assignments. If you collaborate with subcontractors or have team members, you can add multiple performers to the same assignment.

## Adding a Performer

### Step 1: Open the Assignment

Navigate to **Jobs** in the sidebar and click on the assignment. In the assignment detail view, find the performers section.
...
Enter the hourly rate for this performer on this assignment. Key points:
- The rate is specific to this assignment -- the same person can have different rates on different assignments.
- This rate is used to calculate the cost of each time entry (hours x rate = cost).
- For Hourly Rate assignments, this directly affects invoiced amounts.
- For Fixed Price and Retainer assignments, the rate is used for internal tracking and profitability analysis.
...
- **Scope monitoring**: Track if the assignment is on pace to finish within the estimated total hours.
...
Each performer on an assignment has a status:

### Active

The performer is currently working on the assignment. They can:
- Log time entries
...
The performer is temporarily suspended from the assignment. When paused:
...
The performer's involvement in the assignment is complete. When ended:
- They cannot log new time entries
...
If you enable **Automatic Time Tracking** on the assignment, Aourly will automatically create time entries based on each performer's daily workload schedule. This is helpful when:
...
Each performer has their own timesheets for the assignment. Timesheets group time entries by period (weekly or monthly, based on the assignment's timesheet frequency). This means:
...
The assignment dashboard shows progress metrics based on performer workloads:
...
<AccordionGroup>
  <Accordion title="Can I add multiple performers to the same assignment?">
  ...
  <Accordion title="What happens when I deactivate a performer?">
    The performer's status changes to Deactivated. They can no longer log new time entries for this assignment, but their existing entries and timesheets are preserved.
  </Accordion>
  <Accordion title="Can a performer be on multiple assignments at the same time?">
    Yes. A team member can be added as a performer on multiple assignments simultaneously, each with different rates and workload settings.
  </Accordion>
  <Accordion title="What are the performer statuses?">
    Performers can be Active (currently working, can log time), Paused (temporarily suspended, no new time entries), or Ended (permanently removed from the assignment, historical data preserved).
  </Accordion>
