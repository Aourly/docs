...
- Belongs to a **single performer** on a **single assignment**
...
Think of it as a summary sheet for a performer's work on one assignment during one period.

## How Timesheets Are Created

Timesheets are generated automatically. When you log a time entry, Aourly checks:
1. Which assignment and performer the entry belongs to
2. The date of the entry
3. The assignment's timesheet frequency setting
...
The period length depends on the assignment's **timesheet frequency** setting:
...
- **Assignment**: Which job this timesheet is for
...
## One Performer, One Assignment, One Timesheet

An important concept: timesheets are scoped per performer per assignment per period. This means:

- If you have **two performers** on an assignment, each has their own timesheets
- If you work on **three assignments**, you have separate timesheets for each
- If you view a **monthly period**, you see one timesheet per assignment you worked on that month
...
The dashboard provides a high-level overview of your time data across all assignments. It pulls from your timesheets to show:
- Total hours this week/month
- Hours breakdown by assignment
...
<Info>Each performer on an assignment has their own separate timesheets, even for the same period.</Info>

## Frequently Asked Questions

<AccordionGroup>
  <Accordion title="Are timesheets created automatically?">
    Yes. When you log a time entry, it is automatically assigned to the correct timesheet based on the assignment's timesheet frequency and the entry date. You never need to create timesheets manually.
  </Accordion>
  <Accordion title="Can a timesheet span multiple assignments?">
    No. Each timesheet belongs to a single performer on a single assignment. If you work on multiple assignments, you will have separate timesheets for each.
  </Accordion>
  <Accordion title="How does the timesheet period work?">
    The period is determined by the assignment's timesheet frequency setting. For weekly timesheets, the period covers Monday to Sunday. For monthly timesheets, it covers one calendar month.
  </Accordion>
  ...
  <Accordion title="Can I see timesheets for all performers?">
    Owners and coordinators can view timesheets for all performers on their assignments. Performers can only see their own timesheets.
  </Accordion>
</AccordionGroup>
